...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const res = await fetch(`https://boltsearch.aditzerr.workers.dev/api/search?q=${input}`)
  ...
  return (
    <main className='h-screen w-screen grainy'>
      <motion.div className='flex flex-col gap-6 items-center pt-32'
      initial={{y: 20, opacity: 0}}
      animate={{y: 0, opacity: 1}}
      transition={{duration: 0.5}}
      >
        <h1 className='text-5xl tracking-tight font-bold'>BoltSearch ⚡</h1>
